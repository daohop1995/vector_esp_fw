...
int MLX90640_DumpEE(void* slave, uint16_t *eeData)
{
    return MLX90640_I2CRead(slave, 0x2400, 832, eeData);
}

int MLX90640_SynchFrame(void* slave)
{
    uint16_t dataReady = 0;
    uint16_t statusRegister;
    int error = 1;

    error = MLX90640_I2CWrite(slave, 0x8000, 0x0030);
    if (error == -1) {
        return error;
    }

    while (dataReady == 0) {
        error = MLX90640_I2CRead(slave, 0x8000, 1, &statusRegister);
        if (error != 0) {
            return error;
        }
...
int MLX90640_TriggerMeasurement(void* slave)
{
    int error = 1;
    uint16_t ctrlReg;

    error = MLX90640_I2CRead(slave, 0x800D, 1, &ctrlReg);
    ...
    ctrlReg |= 0x8000;
    error = MLX90640_I2CWrite(slave, 0x800D, ctrlReg);

    if (error != 0) {
        return error;
    }

    error = MLX90640_I2CGeneralReset(slave);
...
int MLX90640_GetFrameData(void* slave, uint16_t *frameData)
{
    uint16_t dataReady = 0;
    ...
    error = MLX90640_I2CRead(slave, 0x0400, 768, frameData);
    if (error != 0) {
        return error;
    }

    error = MLX90640_I2CRead(slave, 0x0700, 64, data);
    if (error != 0) {
        return error;
    }

    error = MLX90640_I2CRead(slave, 0x800D, 1, &controlRegister1);
...
int MLX90640_SetResolution(void* slave, uint8_t resolution)
{
    uint16_t controlRegister1;
    int value;
    int error;
    ...
    if (error == 0) {
        ...
        error = MLX90640_I2CWrite(slave, 0x800D, value);
    }
...
int MLX90640_GetCurResolution(void* slave)
{
    uint16_t controlRegister1;
    int resolutionRAM;
    int error;
...
int MLX90640_SetRefreshRate(void* slave, uint8_t refreshRate)
{
    uint16_t controlRegister1;
    int value;
    int error;
...
int MLX90640_GetRefreshRate(void* slave)
{
    uint16_t controlRegister1;
    int refreshRate;
    int error;
...
int MLX90640_SetInterleavedMode(void* slave)
{
    uint16_t controlRegister1;
    int value;
    int error;
...
int MLX90640_SetChessMode(void* slave)
{
    uint16_t controlRegister1;
    int value;
    int error;
...
int MLX90640_GetCurMode(void* slave)
{
    uint16_t controlRegister1;
    int modeRAM;
    int error;
